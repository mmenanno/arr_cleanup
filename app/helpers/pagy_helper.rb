...
module PagyHelper
  ...
  def pagy_tailwind_nav(pagy, pagy_id: nil, link_extra: "", **vars)
    ...
    # Previous button
    html << if pagy.prev
      %(<a href="#{pagy_url_for(pagy, pagy.prev)}" #{link_extra} class="relative inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold text-gray-300 ring-1 ring-inset ring-gray-700 hover:bg-gray-700 cursor-pointer" aria-label="previous">Previous</a>)
    else
    ...
    pagy.series.each do |item|
      ...
      when Integer
        html << if item == pagy.page
          ...
          %(<a href="#{pagy_url_for(pagy, item)}" #{link_extra} class="relative inline-flex items-center rounded-md px-4 py-2 text-sm font-semibold text-gray-300 ring-1 ring-inset ring-gray-700 hover:bg-gray-700 cursor-pointer" aria-label="page #{item}">#{item}</a>)
        end
      when String
    ...
    # Next button
    html << if pagy.next
      %(<a href="#{pagy_url_for(pagy, pagy.next)}" #{link_extra} class="relative inline-flex items-center rounded-md px-3 py-2 text-sm font-semibold text-gray-300 ring-1 ring-inset ring-gray-700 hover:bg-gray-700 cursor-pointer" aria-label="next">Next</a>)
    else
